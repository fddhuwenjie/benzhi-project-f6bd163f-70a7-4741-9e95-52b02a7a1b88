...
func (s *Service) DownloadDossier(caseID string) (*DossierFile, error) {
	s.downloadsMu.Lock()
	if cached, ok := s.downloads[caseID]; ok {
		s.downloadsMu.Unlock()
		return &DossierFile{Reader: cached.reader, Size: cached.size, Filename: cached.filename}, nil
	}
	s.downloadsMu.Unlock()
	...
	cached := cachedDossierDownload{reader: bytes.NewReader(payload), size: int64(len(payload)), filename: safeDownloadName(c.CaseID)}
	s.downloadsMu.Lock()
	if existing, ok := s.downloads[caseID]; ok {
		cached = existing
	} else {
		s.downloads[caseID] = cached
	}
	s.downloadsMu.Unlock()
	return &DossierFile{Reader: cached.reader, Size: cached.size, Filename: cached.filename}, nil
}
...
func (s *Service) VerifyDossier(caseID string) (*DossierVerification, error) {
	...
	return &DossierVerification{Valid: valid, StoredDigest: c.Dossier.ContentDigest, ComputedDigest: computed}, nil
}

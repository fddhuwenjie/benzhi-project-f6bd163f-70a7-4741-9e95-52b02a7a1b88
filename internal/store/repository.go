...
func Open(dir string) (*Repository, error) {
	...
	r.eventFile = file
	r.eventWriter = bufio.NewWriter(file)
	return r, nil
}
...
func (r *Repository) Events(caseID string) ([]EventFrame, error) {
	...
	if r.eventWriter != nil {
		if err := r.eventWriter.Flush(); err != nil {
			return nil, fmt.Errorf("刷新事件日志: %w", err)
		}
	}
...
func (r *Repository) Commit(input CommitInput) error {
	...
	r.mu.Lock()
	defer r.mu.Unlock()
	if !r.integrity.Healthy || r.eventFile == nil || r.eventWriter == nil {
	...
	digest, err := appendFrame(r.eventWriter, frame)
	if err != nil {
		r.integrity.Healthy = false

...
type Repository struct {
	...
	cases       map[string]*domain.DrillCase
}
...
func Open(dir string) (*Repository, error) {
	...
	r := &Repository{dir: dir, casesDir: casesDir, eventsPath: filepath.Join(dir, "events.log"), resultsPath: filepath.Join(dir, "idempotency.json"), results: map[string]StoredResult{}, cases: map[string]*domain.DrillCase{}}
...
func (r *Repository) Load(caseID string) (*domain.DrillCase, error) {
	...
	r.mu.RLock()
	if cached := r.cases[caseID]; cached != nil {
		r.mu.RUnlock()
		return cached, nil
	}
	c, err := readCase(filepath.Join(r.casesDir, filename))
	r.mu.RUnlock()
	if err != nil {
		return nil, err
	}
...
func (r *Repository) Commit(input CommitInput) error {
	...
	r.cases[input.Case.CaseID] = input.Case
	if input.Result != nil {
